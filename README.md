...
요약 프롬프트는 코드가 아니라 `<dataDir>/prompts/summary.md` 에 있다. 파일이 없으면 기동할 때 기본 내용으로 만들고, 이미 있으면 덮어쓰지 않는다. 파일 전체가 요약 요청의 system 메시지이며 요약할 때마다 다시 읽으므로, 고친 내용은 서버를 다시 띄우지 않아도 다음 요약부터 반영된다. Docker 로 띄웠다면 호스트의 `${COWORK_DATA}/prompts/summary.md` 를 고치면 된다. 머리의 마크다운 주석은 모델에 보내기 전에 지워지므로 안내를 적어 두어도 된다. 다만 응답을 `{"summary", "decisions", "actions"}` JSON 으로 파싱하므로 그 형식을 요구하는 문구는 남겨 두어야 요약이 화면에 표시된다. 파일을 지우거나 비우면 코드에 남아 있는 기본 문구로 요약하고 로그를 한 줄 남긴다.
